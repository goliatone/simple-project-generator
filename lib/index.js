...
class Cookiecutter {
    ...
    getPackagePath() {
        ...
        let contentPath = this.template.getContentsPath();
        let packagePath = path.join(contentPath, 'package.json');
        ...
        let targetPath = this.template.getTargetPath();

        /*
         *
         */
        let filepath = path.join(targetPath, this.promptFile);

        let questions = require(filepath);

        if(questions.packagePath) {
            packagePath = path.join(contentPath, questions.packagePath);
            this.packagePath = packagePath;
            return packagePath;
        }

        //@TODO: here we have to walk all of child directories and check
        //for package.json, recursively.
